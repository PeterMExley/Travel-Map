...
export default function GreenlandPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Greenland</h1>

      <img
        src="/photos/north-america/greenland/10.jpeg"
        alt="Greenland"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/north-america/greenland/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/north-america/greenland/${image}`}
              alt={image}
