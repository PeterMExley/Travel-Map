...
export default function SomersetPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Somerset</h1>

      <img
        src="/photos/british-isles/somerset/5.jpeg"
        alt="Somerset"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/british-isles/somerset/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/british-isles/somerset/${image}`}
              alt={image}
