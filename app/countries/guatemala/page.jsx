...
export default function GuatemalaPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Guatemala</h1>

      <img
        src="/photos/north-america/guatemala/10.jpeg"
        alt="Guatemala"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/north-america/guatemala/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/north-america/guatemala/${image}`}
              alt={image}
