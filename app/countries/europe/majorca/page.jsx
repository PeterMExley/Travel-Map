...
export default function MajorcaPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Majorca</h1>

      <img
        src="/photos/europe/majorca/5.jpeg"
        alt="Majorca"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/europe/majorca/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/europe/majorca/${image}`}
              alt={image}
