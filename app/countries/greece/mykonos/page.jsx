...
export default function MykonosPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Mykonos</h1>

      <img
        src="/photos/europe/greece/mykonos/10.jpeg"
        alt="Mykonos"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/europe/greece/mykonos/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/europe/greece/mykonos/${image}`}
              alt={image}
