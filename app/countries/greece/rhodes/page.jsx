...
export default function RhodesPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Rhodes</h1>

      <img
        src="/photos/europe/greece/rhodes/12.jpeg"
        alt="Rhodes"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/europe/greece/rhodes/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/europe/greece/rhodes/${image}`}
              alt={image}
