...
export default function HondurasPage() {
  return (
    <div className="bg-slate-950 min-h-screen text-white p-8">
      <h1 className="text-5xl font-bold mb-8">Honduras</h1>

      <img
        src="/photos/north-america/honduras/1.jpeg"
        alt="Honduras"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {IMAGES.map((image) => (
          <a
            key={image}
            href={`/photos/north-america/honduras/${image}`}
            target="_blank"
          >
            <img
              src={`/photos/north-america/honduras/${image}`}
              alt={image}
